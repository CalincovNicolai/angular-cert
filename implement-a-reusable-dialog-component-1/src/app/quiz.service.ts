...
export class QuizService {
  ...
  createQuiz(categoryId: string | number, difficulty: Difficulty): Observable<Question[]> {
  ...
  private populateSubCategories(categories: Category[]): Category[] {
    ...
    categories.forEach(category => {
      ...
      if (split.length > 1) {
        ...
        const subCategoriesForCategory = catsWithSubCatsMap.get(categoryName) ?? [];
        subCategoriesForCategory.push({ id: category.id, name: split[1].trim() });
